...
int main(int argc, char* argv[]) {
    ...
    if (listen(server_socket, count + 1) < 0) {
        perror("listen");
        exit(EXIT_FAILURE);
    }
    ...
    int monitor = accept(server_socket, (struct sockaddr*)&client_address, &client_address_length);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        int bytes_received = recv(clients[i], str, sizeof(char) * 2048, 0);
        if (bytes_received < 0) {
            perror("recv");
            exit(EXIT_FAILURE);
        }
        ...
        send(monitor, monitor_message, strlen(monitor_message), 0);
    ...
    send(monitor, finalization_msg, strlen(finalization_msg), 0);
